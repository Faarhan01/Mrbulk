...
This project was created with the official `create-medusa-app@2.20.1` DTC starter template using `--with-nextjs-starter`. The initial data seed ran during `db:migrate`.

**Backend is 100% upstream stock.** The `apps/backend/` directory has not been modified since install.

**Storefront has 9 local patches** applied on 2026-09-05 and 2026-09-06:
...
Note: The fresh install and the earlier `medusajstore/myshop` install both ship the same upstream hand-rolled local UI kit in `src/modules/common/components/ui/` (it's part of the upstream starter). The earlier `medusajstore/myshop` install had explicitly **removed** it as a customization; the fresh install keeps it.
...
**What the CLI scaffolds (Medusa 2.20.1, as of 2026-09-05):**
- Root `package.json` does **not** include a `pnpm.overrides` block on Medusa 2.20.1 (this was fixed in later versions). The overrides are correctly placed in `pnpm-workspace.yaml` already. **No `pnpm.overrides` warning** is triggered.
...
This install already has the `@types/react` and `@types/react-dom` overrides correctly placed in `pnpm-workspace.yaml` (lines 12-14). **No `pnpm.overrides` block exists in the root `package.json`** — it was never there in the 2.20.1 scaffold. No action needed.
...
The 2.20.1 scaffold ships with several pre-existing bugs. See the full bug catalog in `customizations/known-issues.md`:
...
`pnpm run lint` in `apps/storefront` reports **8 errors and 3 warnings**, all in upstream code. See `customizations/frontend-fixes.md` for the full list (BUG-08 through BUG-12). These are dev-time warnings only — the build, dev server, and prod server all run fine despite them.
...
- **The scaffold already handles BUG-01 (orders auth gate), BUG-07 (profile-email no-op), BUG-13 (gitignore), BUG-14 (next.config.js), and BUG-02 through BUG-06, BUG-15 (UI patches).** No further action needed on these.
...
- **Storefront has 9 local patches.** BUG-01 (orders auth gate), BUG-02 (modal max-h), BUG-03/BUG-04 (address modal size), BUG-05 (billing type), BUG-06/BUG-15 (overflow-hidden), BUG-13 (.gitignore), BUG-14 (next.config.js qualities). Everything else is the upstream DTC starter.
- **The `pnpm.overrides` field is NOT in `package.json`** — it was never scaffolded in the 2.20.1 scaffold. The overrides are correctly in `pnpm-workspace.yaml`.
- **`.npmrc`**: The CLI scaffolds a root `.npmrc` with `auto-install-peers=true`. No `public-hoist-pattern` is required for Medusa 2.20.1 — the duplicate `@types/react` issue is resolved via `pnpm-workspace.yaml` overrides, not via hoisting. Do not add `public-hoist-pattern` speculatively.
...
These are guaranteed to appear on a clean `create-medusa-app@latest --with-nextjs-starter` install with Medusa 2.20.1, regardless of platform or setup mistakes:
...
| `GET /store/locales 404` in dev log | `next dev` console | Noise | The v2.20.1 storefront probes a `locales` endpoint that doesn't exist in this version. Benign. |
...
| 8 errors + 3 warnings from `pnpm run lint` | `cart.ts`, `language-select`, shipping, etc. | Noise | BUG-08 through BUG-12. Dev-time lint only. See `customizations/frontend-fixes.md` for details. DO NOT change empty stubs in `cart.ts` to `throw new Error(...)`. |
...
**Cause**: v2.20.1 storefront still issues a `locales` probe against the store API; the endpoint isn't routed in this version.
**Fix**: Benign; ignore.
...
| `MEDUSA_CLOUD_S3_PATHNAME` | Medusa Cloud S3 bucket path (optional) | empty |
...
- `customizations/known-issues.md` — full bug catalog with BUG-01 through BUG-15 IDs, status, and fix details
...
*Generated from the fresh install experience with MedusaJS v2.20.1 on Windows + PostgreSQL 18 + pnpm 11 + Node v24, using `create-medusa-app@2.20.1` with `--with-nextjs-starter`.*