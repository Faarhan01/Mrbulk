...
│  ┌────────────────────────────────────────────────────────┐  │
│  │ grid grid-cols-1 small:grid-cols-[240px_1fr]           │  │
│  │                                                        │  │
│  │  ┌──────────┐  ┌─────────────────────────────────┐    │  │
│  │  │ AccountNav│  │  Active slot content              │    │  │
│  │  │ (240px)  │  │  (profile / addresses / orders)   │    │  │
│  │  └──────────┘  └─────────────────────────────────┘    │  │
│  │                                                        │  │
│  │  ┌──────────────────────────────────────────────────┐  │  │
...
- Sidebar: `240px` fixed width on `small:` screens (≥1024px), hidden on mobile.
...
- **Profile completion**: large percentage badge with label.
- **Addresses**: count of customer addresses + "Saved" label.
- **Recent orders**: up to 5 most recent orders, each showing date placed, order number, and total amount.
...
`@login/page.tsx` renders `<LoginTemplate>` which contains `<Login>` and `<Register>` side by side.

- `<Login>` uses `useActionState` with `login()` from `@lib/data/customer`.
- `<Register>` uses `useActionState` with `signup()` from `@lib/data/customer`.
- On success, Register shows a success banner and hides the form. Login redirects via the action.
